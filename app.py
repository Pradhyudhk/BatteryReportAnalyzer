...
from src.core.parser import parse_battery_report
from src.utils.date_helpers import get_week_range
...
class BatteryReportApp(QMainWindow):
    ...
    def load_report(self):
        ...
        try:
            battery_data, debug_log = parse_battery_report(file_path)
            self.battery_data = battery_data
            self.debug_log = debug_log
        except Exception as e:
            self.degradation_stats.setText(f"⚠️ {str(e)}")
            return
    ...
    def update_week_combo(self):
        ...
        while current_date <= datetime(selected_year, selected_month, last_day):
            start_date, end_date = get_week_range(selected_year, selected_month, week_num)
